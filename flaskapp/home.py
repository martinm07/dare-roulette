...
def get_dares():
    rows = db.session.scalars(select(Dare).where(Dare.used == False)).all()
    print(rows)
...
def pick_user():
    rows = db.session.scalars(
        select(Dare.by).where(Dare.used == False).distinct()
    ).all()
    print(rows)
...
def set_played():
    ...
    test = db.session.scalars(
        select(Dare.id).where(and_(Dare.used == False, Dare.played == False))
    ).first()
    print("🎈🎈🎈", test)
    if test is None:
        print("Updating all dares to be not played")
        dares = db.session.scalars(select(Dare).where(Dare.used == False)).all()
        print(dares)
        for dare in dares:
            dare.played = False
